...
'''
    ...
    if mode == 1:   
    pdb_data=[]
        for root, names, files in os.walk(input):
            for IDs in names:
                pdb_path = os.path.join(root, IDs)
                name=IDs+'_complex.pdb';
                print(name)
                pdb_path = os.path.join(pdb_path, name)
                print(pdb_path)
...
if __name__ == "__main__":
    ...
    help='input: in mode 1, means the the FODER path of pdb complexes(./pdb),in /data/cdk2/.maegz ],all pdb files need to be unpacked to release; \
        in mode 0, means the FILES path of smiles (./zinc.csv)')
